...
def upgrade() -> None:
    ...
    op.create_index(
        ...
        "attempts",
        ["quiz_id", sa.text("score DESC"), sa.text("total_time_ms ASC"), sa.text("finished_at ASC")],
        unique=False,
    )
    ...
    op.create_table(
        "attempt_answers",
        sa.Column("id", sa.BigInteger(), primary_key=True),
        sa.Column("attempt_id", sa.BigInteger(), sa.ForeignKey("attempts.id", ondelete="CASCADE"), nullable=False),
        sa.Column("question_id", sa.BigInteger(), sa.ForeignKey("questions.id", ondelete="CASCADE"), nullable=False),
...
def downgrade() -> None:
    ...
    op.execute("DROP TYPE IF EXISTS text_match_type")
